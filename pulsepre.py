...
class PulsePre:
	def __init__(self):
		...
		self.fig.canvas.set_window_title('Predicted Heart Rate')
	...
	def plotpulse(self,num):
		...
		trainX,trainY,testX,testY,oritestX,scaler=self.updateData(num)
		if num==-201: 
			self.model.append(self.trainmodel(trainX,trainY,testX,testY,scaler))

		futurePredict=self.predictapp(num,oritestX,self.model[0],scaler)
		self.p(self.x1,futurePredict)
